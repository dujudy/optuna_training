...
def fill_objective(type, feats, labs, input_df, scoring_metric, model_name):
  def filled_obj(trial):
    return optuna_via_sklearn.specify_sklearn_models.objective(trial,type, feats, labs, input_df, scoring_metric, model_name)
  return filled_obj

def optimize_hyperparams(feature_type, scoring_metric, n_trials,  model_name):
    specified_objective = fill_objective(feature_type, features, labels, input_df, scoring_metric,  model_name)
...
if __name__ == "__main__":
    ...
    features, labels, input_df, metadata, feature_columns = load_data(config)
    ...
    run_id = args.results_folder + "/" + "{write_type}" + args.language_model_type + "_" + args.feature_type + "_" + args.model_name + args.scoring_metric
